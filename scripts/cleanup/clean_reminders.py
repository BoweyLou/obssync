...
def confirm_deletion(list_name: str, task_count: int) -> bool:
    ...
    print(f"\n⚠️ This will permanently delete {task_count} tasks from '{list_name}'.")
    print("🔄 This action cannot be undone. Make sure you really want to continue.")
    print("💾 Confirm you have a backup in case you need to restore these tasks.")
...
def main():
    ...
    print("🧹 Apple Reminders Clean Slate")
    print("=" * 50)
    ...
    if args.list_id:
        ...
        if not list_ids:
            print("❌ No reminder lists are configured and no --list-id was provided.")
            print("💡 Run 'obs-sync setup' first or provide --list-id.")
            return 1
    ...
    try:
        ...
        print("💡 Ensure obs-sync has permission to access Apple Reminders.")
        return 1
    ...
    if total_deleted == total_tasks_to_delete:
        print("🎉 All targeted Apple Reminders lists are now clean.")
        print("💡 Run a fresh sync to rebuild tasks with deterministic UUIDs.")
    else:
        print("⚠️ Some tasks could not be deleted—review the details above.")
        return 1
        return 1
